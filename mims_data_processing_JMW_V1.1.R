...
save_file_name <- "MIMS_Data_Log_2024-11-12_stats.csv"
...
drift_corr <- function(parsed_data, std_data, meas_col, i) {
  ...
  # Retrieve time index of first sample meansurment in data subsample
...
get_del_15N <- function(parsed_data) {
  ...
  for (i in 1:(length(unique(parsed_data$Calibnum)) - 1)) {
    ...
    # Average 29.28 ratios for standards
    X29.28_std <- mean(calibs[["X29.28"]])
    
    # Calculate del_15N value for samples
