# """"
# Author: Jamison Ward
# Created: Wed Nov 13 11:38:51, 2024
# """"
...
save_file_name <- "MIMS_Data_Log_2024-11-14_stats_noDC.csv"
...
gather_data <- function(raw_file, metadata_file, pressure_unit) {
  ...
  for (j in unique(metadata$Samp)) {
    ...
    new_data <- lapply(parsed_data, as.numeric)
    ...
    calc_data <- data.frame(
      ...
      "Num.Meas" = indices[[2]] - indices[[1]] + 1
      )
...
################################################################################
## Apply instrument drift correction to sample measurement data ##

drift_corr <- function(parsed_data, std_data, meas_col, i) {
  
  # Fit a linear model to the standard measurements versus time (index)
  drift_model <- lm(std_data[[meas_col]] ~ std_data[["Index"]])
  
  # Retrieve drift rate from linear regression
  drift_rate <- coef(summary(drift_model))[2]

  # Retrieve sample data
  spl_data <- parsed_data %>%
    filter(!grepl("std", tolower(SampleID)) & Calibnum == i)

  # Retrieve time index of first sample measurement in data subsample
  init_time <- min(spl_data$Index[spl_data$Calibnum == i], na.rm = TRUE)

  # Calculate correction factors for each sample measurement
  correction_factors <- drift_rate * (spl_data$Index - init_time)

  # Apply correction factors to each sample measurement
  spl_data[[meas_col]] <- spl_data[[meas_col]] - correction_factors
  
  # Overwrite sample measurements in parsed data set with corrected measurements
  parsed_data[[meas_col]][!is.na(base::match(parsed_data$Index, spl_data$Index))] <- spl_data[[meas_col]]
...
get_N2_conc <- function(parsed_data) {
  ...
  for (i in 1:(length(unique(parsed_data$Calibnum)) - 1)) {
    ...
    # Correct sample measurement data for instrument drift
    sub_samp <- drift_corr(sub_samp, calibs, targ_col, i)
    ...
    parsed_data[[targ_col]][!is.na(base::match(parsed_data$Samp, sub_samp$Samp))] <- sub_samp[[targ_col]]
...
get_del_15N <- function(parsed_data, R_AIR) {
  
  # Define the target column (targ_col)
  targ_col <- "X29.28"
  ...
  for (i in 1:(length(unique(parsed_data$Calibnum)) - 1)) {
    ...
    # Average 29.28 ratios of standards
    X29.28_std <- mean(calibs[["X29.28"]])
    
    # Calculate del_15N value for std-AIR
    del_15N_std <- (((X29.28_std / 2) / R_AIR) - 1) * 1000
    
    # Calculate del_15N value for spl-std
    sub_samp[[new_col]] <- ((sub_samp[[targ_col]] / X29.28_std) - 1) * 1000
    
    # Calculate del_15N value for spl-AIR
    sub_samp[[new_col]] <- sub_samp[[new_col]] + del_15N_std + (sub_samp[[new_col]] * del_15N_std / 1000)
...
get_del_18O <- function(parsed_data, R_VSMOW) {
  ...
  for (i in 1:(length(unique(parsed_data$Calibnum)) - 1)) {
    ...
    # Average 34.32 ratios of standards
    X34.32_std <- mean(calibs[["X34.32"]])

    # Calculate del_18O value for std-VSMOW
    del_18O_std <- (((X34.32_std / 2) / R_VSMOW) - 1) * 1000
    
    # Calculate del_18O value for spl-std
    sub_samp[[new_col]] <- ((sub_samp[[targ_col]] / X34.32_std) - 1) * 1000
    
    # Calculate del_18O value for spl-VSMOW
    sub_samp[[new_col]] <- sub_samp[[new_col]] + del_18O_std + (sub_samp[[new_col]] * del_18O_std / 1000)
...
get_data_stats <- function(parsed_data) {
  ...
  meta_cols <- c("SampleID", "Pressure", "Temp", "Calibnum", "Depth", "WatDens",
                 "N2Sat", "O2Sat", "ArSat", "N2.ArSat", "O2.ArSat", "Num.Meas")
  meas_cols <- setdiff(colnames(parsed_data), c("Index", "Samp", meta_cols))
  ...
  meas_col_names <- colnames(results)[grepl("_mean$|_sd$|_sem$|_ci_lower$|_ci_upper$", colnames(results))]
  new_names <- paste0(rep(meas_cols, each = 5), 
                      c("_mean", "_sd", "_sem", "_95ci_lo", "_95ci_up"))
...
R_AIR = 3.677e-3  # 15N/14N isotope ratio of AIR
raw_data_processed <- get_del_15N(raw_data_processed, R_AIR)

# Get sample del_18O values
R_VSMOW = 2.0052e-3  # 18O/16O isotope ratio of VSMOW
raw_data_processed <- get_del_18O(raw_data_processed, R_VSMOW)
...
write.csv(raw_data_stats, paste0(output_file_path, save_file_name), quote = TRUE)
